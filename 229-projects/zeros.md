...
* the call `roots(f)`  finds all roots of a polynomial function, even complex ones

* the call `fzeros(f)` finds all *real* roots of polynomial function

* the call `fzero(f, [a,b])` finds a root of a function between a **bracketing** interval, `[a,b]`, using the bisection method.

* the call `fzeros(f, [a,b])` function *attempts* to find all roots of a function in an interval `[a,b]`. This may miss values; answers should be checked graphically.
...
roots are even close together? Are all three roots still found by `fzeros`?
