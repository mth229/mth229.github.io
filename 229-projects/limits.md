...
From this we see a *right* limit at 0 appears to be $1$. (Look at what happens to the second column.)
...
val = N(limit(f, 3))
numericq(val, 1e-2, L"limit of $\frac{x^3 - 2x^2 -9}{x^2 - 2x -3}$")
```
...
* Find the *left* limit of `f(x) = cos(pi/2*(x - floor(x)))` as $x$
   goes to $2$. 
...
ans = 3
radioq(choices, ans, "which k does the limit not exist", keep_order=true)
```

* Let `l(x) = (a^x - 1)/x` and *define* $L(a) = \lim_{x\rightarrow 0} l(x,a)$.
