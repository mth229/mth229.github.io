...
(Hint, let `t1` and `t3` be defined using `tangent(f,c)` for the appropriate `c` and then apply `find_zero` to `aux(x) = t1(x) -  t3(x)` starting at $1$.)
