...
[(Pluto html)](./229-projects/07-newton.html)
[(With commentary)](./229-projects/07-newton-pluto-commentary.html)
...
compose(f, n) = n <= 0 ? identity : f ∘ compose(f, n-1)
ss = "₁₂₃₄₅₆₇₈₉"

f(x) = cbrt(x)
nm(x) =  x - f(x)/f'(x)
x1 = 1
xs = [compose(nm,i)(x1) for i ∈ 0:5]

newton_vis(cbrt, x1, steps=5)
scatter!(xs, 0 .* xs)
annotate!([(xᵢ, 0, text("x$(ss[1 + 3(i-1)])",:top)) for (i,xᵢ) ∈ enumerate(xs)])
```
