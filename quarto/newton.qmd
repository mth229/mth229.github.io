...
The `MTH229` package provides a function `newton` for easily performing Newton's method, utilizing a function from the `Roots` package. More usefully, we will see that `fzero`, which we used for bisection, can also be used for root finding with an algorithm that is similar, but a bit more robust than Newton's method.
...
That is, most functions at any given point are well approximated by the tangent line at that point. If we have a good, *nearby*, guess $x_n$, then we can improve this guess by replacing it with the easy to find zero, $x_{n+1}$, of the tangent line to $f(x)$ at $(x_n, f(x_n))$.
...
scatter!([x1], [f(x1)])
annotate!([(x1, 0, text(L"x_n",:bottom)), (x2, 0,  text(L"x_{n+1}",:bottom)),
           (x1, f(x1), text(L"(x_n, f(x_n))",:left)),
            (1,0, text(L"x^\alpha", :top))])
```
...
The basic algorithm of Newton's methods solves this for $x_{n+1}$ to get:
...
Like the bisection method, Newton's method is an *iterative method*. One begins with a (suitable, ideally *nearby*) guess $x_1$. From that the algorithm produces $x_2$ which is used to produce $x_3$, etc. The idea is that the algorithm, when successful, will  eventually will settle on a value $x_n$ sufficiently close to the desired root. We then call $x_n$ an *approximate* zero, $x^\alpha$ being the exact mathematical zero.
...
However, the `MTH229` package provides the `newton` function. So we shall use that in the sequel.
...
There are also very fast algorithms which do not require a derivative. The `Roots` package provides an interface to these through the `fzero` function.
...
Many bracketing methods (like bisection) are guaranteed to converge, but can be slow. The iterative algorithm used by default with `fzero` tries to speed the convergence up, but if along the way it finds a bracketing interval, that will guarantee convergence.
...
##### Example: using a symbolic equation

As illustrated in the section on the bisection algorithm, symbolic equations can be specified using `~` to separate the left- and right-hand sides, which are symbolic expressions.

The following would be an alternate to using the `aux` method above:
...
Let $f(x) = x^2 - 3x + 5$. Use `fzero` to find the intersection point of the tangent line at $1$ and the tangent line at $3$. Where does this happen?


(Hint, let `t1` and `t3` be defined using `tangent(f,c)` for the appropriate `c` and then apply `fzero` to `aux(x) = t1(x) -  t3(x)` starting at $1$.)
...
Despite all our care with the derivative, the method did not converge in $200$ steps. Can you see why from this trace, which starts at $x_0=3$?
...
#plot!(zero)
newton_vis(f, x0; steps=5, annotate_steps=6)
```
