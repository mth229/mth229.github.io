...
```{julia}
#| fig-alt: plot of f with zero line added.
...
```{julia}
#| fig-alt: plotif graph showing where the function f is non-negative
plotif(f, f, -2, 1)
```
...
### Numerically find zeros of a function over $[a,b]$; the `fzeros` function
...
* When a zero is *near* a value, such as these four which are near $-1.5$, $-0.75$, $0.0$ and $0.8$. Such zeros may typically be found with *Newton's* method (or `fzero`).

The `Roots` package, exported with `MTH229`, provides a convenience function, `fzeros`, which uses bisection or `fzero` to identify *heuristically* all zeros in an interval $[a,b]$. It is easy to use: the interval is specified just as is done with plotting:

```{julia}
fzeros(f, -2, 1)  # note the s in fzeros
```

The algorithm isn't guaranteed to always work the way bisection is, but works fairly well to identify all the zeros except for cases where there are *many* zeros in the interval or the interval chosen is *too wide*. (A bracketing interval specified to bisection may be infinite, but such wide intervals can be problematic with `fzeros`, though can work in some cases.)
...
The algorithm used by `fzeros` relies on both bisection and a method like Newton's method. The bisection implementation will pick up on places where the function changes sign, even if not continuous. In the example below, the two vertical asymptotes at $3$ and $4$ are found:
...
The technique above to find where a function is positive can be automated if `fzeros` is used to find the zeros. This is done in `sign_chart`.
...
The output shows how the sign changes at the identified zeros (or other points). In this case we can infer that the function is positive on $(-0.8656\dots, 1.22688869\dots)$ and again on $(16.998887\dots, 20)$. Furthermore, it can be inferred that between $(-1, -0.8656\dots)$ the function is *negative* and as well in $(1.22688869\dots, 16.998887\dots)$.
...
```{julia}
#| echo: false
val = length(fzeros(x -> exp(-x/2)*(sin(11x) + sin(13x)), -2, 1))
numericq(val)
```

#### Question

What is the largest value identified in $[-2,1]$ by `fzeros` for the function
...
```{julia}
#| echo: false
val = maximum(fzeros(x -> exp(-x/2)*(sin(11x) + sin(13x)), -2, 1))
numericq(val)
```
...
```{julia}
#| fig-alt: plot of polynomial x^3 - 2x - 1/2 showing where it is non-negative
f(x) = x^3 - 2x - 1/2
plotif(f, f, -2, 2)
plot!(zero)
```
...
```{julia}
#| fig-alt: plot of polynomial x^3 - 2x - 1/2 showing where it is increasing, as its derivative is positive
plotif(f, f', -2, 2)
```
...
```{julia}
#| fig-alt: plot of polynomial x^3 - 2x - 1/2 showing where it is concave up, as its second derivative is positive
plotif(f,  f'', -2, 2)
```
...
The `fzeros` function can readily identify when some function is zero (or undefined). It can be used, as seen, to find zeros. However, if passed `f'` it will identify critical points. When passed `f''` it will find when $f''(x) = 0$ (or undefined). Such points are *candidates* for inflection points, though a check on whether the concavity changes is necessary to say such is an inflection point.

In the following we use `scatter!` to show these values on the graph.

```{julia}
#| fig-alt: plot of f with zeros, critical points, and inflection points added
...
```{julia}
#| fig-alt: plot of exp(x) over 0,4 showing it is always positive
fp(x) = exp(x)
plotif(fp, fp, 0, 4)
```

```{julia}
#| fig-alt: plot of exp(x) over 0,4 showing ii iss always increasing
plotif(fp, fp', 0, 4)
```
...
```{julia}
#| fig-alt: plot of x^2 -2x  over -1, 3 showing where it is concave up
fpp(x) = x^2 - 2x
plotif(fpp, fpp, -1, 3)
```
...
As explained, we could use `fzeros` to identify these more precisely:
