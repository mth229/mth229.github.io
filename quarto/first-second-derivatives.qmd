...
The `Roots` package, exported with `MTH229`, provides a convenience function, `fzeros`, which uses bisection or `fzero` to identify *heuristically* all zeros in an interval $[a,b]$. It is easy to use: the interval is specified by pairing off the interval:
