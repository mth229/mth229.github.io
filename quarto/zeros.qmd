...
We have the `h(x) = f(x) - g(x)` approach:
...
The alternate form is to define our function with a *parameter*:
...
[^1]: We used a keyword argument in the example code, the last positional argument can also be used, which is useful for broadcasting, were we interested in find solutions for multiple values of the paramter.
