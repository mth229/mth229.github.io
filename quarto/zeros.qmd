...
The `find_zero` function finds a value solving $f(x) = 0$. When the question involves two function, e.g. $f(x) = g(x)$ it can't be immediately used. However, forming an auxiliary function $aux(x) = f(x) - g(x)$ works as solutions to $aux(x) = 0$ satisfy $f(x) = g(x)$.
...
The `MTH229` package loads the `SimpleExpressions` package which provides a means to create basic symbolic variable (and parameter) through the macro `@symbolic`. `SymPy` will uses the more powerful `@syms` macro for a related task.
