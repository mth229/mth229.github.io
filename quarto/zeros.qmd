...
## The Roots package and find_zero to solve $f(x) = 0$
...
From the graph, we confirm that $[0,1000]$ contains a zero and is a bracketing interval. With this, we are ready to call `find_zero`:
...
The `find_zero` function finds a value solving $f(x) = 0$. When the question involves two functions, e.g. $f(x) = g(x)$ it can't be immediately used. However, forming an auxiliary function $aux(x) = f(x) - g(x)$ works as solutions to $aux(x) = 0$ satisfy $f(x) = g(x)$.
...
With a symbolic variable, computed expressions stay symbolic until needed.  Symbolic expressions can be paired together with a `~` to create a *symbolic equation*. Such equations can be `solve`ed with the `solve` function. While `SymPy` can solve symbolic equations for algebraic answers, `SimpleExpressions` uses `find_zero` behind the scenes.
...
sl_12 = secant(f, 1, 2)
sl_21 = secant(f, -2, -1)
find_zero(sl_12(x) ~ sl_21(x), (-Inf, Inf))
```
