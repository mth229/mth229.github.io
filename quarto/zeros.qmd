...
```{julia}
f(x) = exp(x)
h(x) = secant(f, 1, 2)(x) - secant(f, -2, -1)(x)
find_zero(h, (-Inf, Inf))
```

The above follows the pattern of using `h(x)= f(x) - g(x)` to solve $f(x) = g(x)$. But the two functions have a somewhat awkward syntax. The `secant(f, 1,2)` call returns a function, so `secant(f, 1, 2)(x)` is used to call that function at `x`.
