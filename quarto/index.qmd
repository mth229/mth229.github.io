...
available as `IJulia` notebooks. (The `Pluto` notebooks are also available, though those haven't been updated in a while. These have recordings with 5-10 minutes of commentary, should that prove interesting.) Find links for
...
* `fzero(f, (a, b))` or `fzero(f, a, b)` to find *a* zero *between*  the bracketing interval `[a,b]` (from `Roots`)
* `fzero(f, a)` (or `fzero(f, a)`) to find *a* zero *near* an  initial starting point `a` (from `Roots`)
...
* `fzeros(f, (a, b))` (or `fzeros(f, a,b)`) to find *all* the real zeros of a function `f` in `[a,b]` (from `Roots`)
