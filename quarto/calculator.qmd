...
The programming language `Julia` (www.julialang.org) is a relatively new programming language that builds on a long history of so-called dynamic scripting languages (DSLs).  DSLs are widely used for exploratory work and are part of the toolbox of most all data scientists, a rapidly growing area of employment. The basic `Julia` language is reminiscent of all of `MATLAB`, `R`, `python`, `LISP`, and even `Ruby`, though it has differences from each of these languages. For these notes, the focus will be on its more transparent syntax, but in general a major selling point of `Julia` is that it is much performant than the other languages at many tasks.
...
In this project we start with baby steps---how to use `Julia` to perform operations we can do on the calculator. The use of a calculator requires knowledge about several things, often taken for granted
...
We know that the subtraction needs to be done before the division, as this is how we interpret this form of division (with the horizontal bar). How to make this happen? The precedence of parentheses is used to force the subtraction before the division, as in `(12-10)/2`. Without parentheses you get a different answer:
...
What about the case where we have different operations with the same precedence, what happens then? A simple example would be $2 / 3 \cdot 4$? Is this done in a left to right manner as in:
...
There are  notations in math notation where there is *implied grouping*, one of which requires attention when translating mathematical notation into `Julia` commands.

The notations use a *vinculum*, a horizontal line used in mathematical notation  placed as an overline or underline above or below a mathematical expression to group the expression's elements (wikipedia definition).

The main notation is when the vinculum is used for division, as in

$$
\frac{1 + 2}{3 + 4}
$$

That long bar implies that *both* the numerator and denominator are evaluated *before* the division. That is this is a *mathematical* equivalent:

$$
(1 + 2) / (3 + 4)
$$

With that in mind, always use parentheses when transcribing this notation *when* there is some operation indicated in the numerator or denominator.
...
we would use `sin((1 + x) / x)`. The denominator has no operation, so we need not group the last `x`.

#### Exponents

Another place where there is implied grouping in math notation is the use of superscripts to indicate powers. The expression in the exponent must be grouped with parentheses. That is, this expression, that might be found in a probability question:
...
Forgetting the parentheses, as in `e^1/2*(4-3)^2` would be equivalent to just $e^1 \cdot (1/2)\cdot(4-3)^2$.

For a base of $e$, the use of function `exp` avoids this point, as to pass an argument to a function requires parentheses already. So the above could be `exp(1/2*(4-3)^2)`. (As well, `e` is not exposed in base `Julia`, though the `MTH229` package defines it through `e=exp(1)`.)
...
Most all calculators used are not limited to these basic arithmetic operations. So-called scientific calculators provide buttons for many of the common mathematical functions, such as the exponential, logarithmic, and trigonometric functions. `Julia` provides these too, of course.
...
This simple computation, or function call, shows how to evaluate a function:

* Functions are referred to by name (`sqrt`).
* Arguments are passed to the function inside parentheses (`15`).
* Between the function name and the opening parentheses there is *no* space.
* Functions may have $0$, $1$, or more arguments. When there are multiple, they are separated by a comma. (This call had 1.)
* There are *positional* arguments and *keyword arguments* the latter specified through a `key=value` format. (This call only uses a positional argument.)
...
(For simplicity, we fall back on direct conversion in the sequel.)
...
(The constant value of `e` is only available with the `Base.MathConstants` module. The `MTH229` package, which is used in subsequent sections, defines `e` as `exp(1)`, for convenience.)
...
With a calculator, one can store values into a *memory* for later usage. This useful feature with calculators is greatly enhanced with computer languages, where one can bind, or assign, a value to an identifier, or variable. For example the command `x=2` will bind `x` to  the value $2$:
...
The basic tradeoff being: longer names are usually more expressive and easier to remember, whereas short names are simpler to type. Not all names are syntactically valid, for example names can't begin with a number or include spaces.


:::{.callout-note}
## Alert
In fact, only *most* objects bound to a name can be arbitrarily redefined. When we discuss functions, we will see that redefining generic functions as values (or vice versa) can be an issue and new names will need to be used. As such, it often works to stick to come convention for naming: numbers use values like `i`, `j`, `x`, `y`; functions like `f`, `g`, `h`, etc.

:::

#### The equals sign differs between mathematics and Julia
...
This isn't the case for values stored as integers, but is the case for *floating point* values, which are only exact when a number can be written as a sum of powers of $2$.
...
Maybe more surprisingly, the following very simple mathematical calculation won't be exact:
...
Comparisons with `==` do a promotion prior to comparing, so even though these numbers are of different types, the `==` operation treats them as equal:
...
For most purposes, testing equality with `==` is appropriate.
...
The `e` in scientific notation is **not** the $e$ used in mathematics. The `e` in scientific notation specifies a format that separates a *mantissa* (a normalized number in $[1,10)$ and an exponent (for a base of $10$). For 64-bit numbers, the printed version represents the bits that store these parts. There are 52 bits used for the mantissa, 11 for the exponent, and 1 for a sign.
...
These values can come up in unexpected circumstances. For example division by $0$ can occur due to round-off errors, especially when subtracting like-sized quantities:
...
Another special floating point number is `-0.0`, which is technically different from `0.0` though the following holds:

```{julia}
-0.0 == 0.0  # separate but equal...
```
...
Rational numbers are used typically to avoid round-off error, that can arise using floating point values. As `Julia` promotes values to a common type, rational numbers easily be mixed into expressions:
...
Complex numbers are an extension of the real numbers when the values $i = \sqrt{-1}$ is added. Complex numbers have two terms: a real and imaginary part. They are typically written as $a + bi$, though the polar form $r\cdot e^{i\theta}$ is also used. Complex numbers have the usual arithmetic operations defined for them.
...
We see that `Julia` uses `im` (and not `i`) for the $i$. It can be more direct to just use this special constant in constructing complex numbers:
...
Complex numbers have a big role to play in higher level mathematics. In calculus, they primarily occur as roots of a polynomial.
...
If you need more bits, `Julia` provides the `BigInt` class which allow for what is known as [*arbitrary precision arithmetic*](https://en.wikipedia.org/wiki/Arbitrary-precision_arithmetic), where the number of bits used to store the number grows as needed.
Using this allows one to compute $2^{3^4}$ precisely as an integer:
...
:::{.callout-note}
## Alert
Like the `BigInt` class, `Julia` also has a `BigFloat` class for storing decimal numbers with arbitrary precision.
Due to some technical details of how floating point numbers are stored, the number of bits must be fixed in advance of any computation—unlike `BigInt`s whose precision can grow on-the-fly.
`Julia` provides the `setprecision` command to set this number of bits (e.g. `setprecision(1024)`). The default precision is $256$ (four times the size of `Float64`).
:::
----
