...
Another place where there is implied grouping in math notation is the use of superscripts to indicate powers. The expression in the exponent must be grouped with parentheses. That is, this expression, that migth be found in a probability question:
...
For a base of $e$, the use of function `exp` avoids this point, as to pass an argument to a function requires parentheses already. So the above could be `exp(1/2*(4-3)^2)`
...
This shows how to evaluate a function:

* Functions are referred to by name.
* Arguments are passed to the function inside parentheses.
* Between the function name and the opening parentheses there is *no* space.
* Functions may have $0$, $1$, or more arguments. When there are multiple, they are separated by a comma.
* There are *positional* arguments and *keyword arguments* the latter specified through a `key=value` format.
...
(The `MTH229` package, which is used in subsequent sections, defines `e` as `exp(1)`, for convenience.)
...
`Julia` provides the `setprecision` command to set this number of bits (e.g. `setprecision(1024)`). The default precision is $256$.
:::
----
