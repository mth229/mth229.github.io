...
The `Julia` language is a relatively new language and as such, certain design decisions are still being made. One key decision is the interface for creating graphics. At this point there are many different ones (`Makie`, `PyPlot`, plotly, plotlyjs, `GR`, `Winston`, `Gadfly`, `Gaston`,...), and perhaps more will be generated before a dominant one is arrived at. As such, we don't try to teach the details of any one of them.


Currently, the  `Plots` package  provides a unified interface to many backend plotting packages.  We will use the popular `Plots` package in these examples, but recognize that it is likely that `Makie` will eventually become the preferred plotting package.


For the impatient, this is all that is needed to know to get up and running to make a plot.

First load the necessary package and set a backend:
...
### Adding layers using "plot!", "scatter!"
...
The `Plots` packages uses *positional* arguments for data (like the function and domain to plot over) and *keyword* arguments to adjust plotting attributes. When viewing multiple plots, there are a few adjustments to the default attributes that can clarify the graphic for the user, all specified with keyword arguments.

* A legend is automatically included. The different layers added to the graphic can be included in the legend with a given name by passing the keyword argument `label="name to give"` to each layer.
* The legend can be suppressed by passing the keyword `legend=false` to the initial `plot` command
* The color of a new line can be specified with `linecolor="color"`. The color can be specified by name in many cases
* The width of a new line can be specified with `linewidth=n` where `n` is the pixel width. A width of 5 is already kinds prounounced.
* The style of the line can be specified with `linestyle=style`. There are many styles, but `:dash`, `:dot`, and `:dashdot` are useful. (These are "symbols" and start with a leading colon.
* For `scatter!` the markers can be adjusted using `markercolor`, `markersize`, and `markershape`.
...
The `MTH229` package implements this in the function `rangeclamp(f, hi=20, lo=-hi)`. (Base `Julia` has a `clamp` function.) We first need to load the package before having access to any features:
...
The range of `x` values comes from the choice of `a` and `b` in `plot(f, a, b)` and the range of `y` values comes from the range of `f` over the interval `[a,b]`. These can be adjusted manually through the keyword arguments `xlims=(l,r)` and `ylims=(m, M)`. An alternate to clamping `f` is to change the `y` limits, though as no `NaN` values are used, there can be artifacts in the resulting graphs:
...
A rational function is nothing more than a ratio of polynomial functions, say $f(x) = p(x)/q(x)$. One interesting thing about such function is there can be asymptotes. These can be vertical (which can happen when $q(x)=0$), horizontal (as $x$ gets large or small), or even slanted (when $p(x)$ and $q(x)$ have the same degree).


The vertical asymptotes require care when plotting, as the naive style of plotting where a collection of points is connected by straight lines can render poor and even misleading graphs when the scale of $y$ values get too large. The really large values plotted near the asymptote can wipe out the possibility of seeing other features of interest in a graph.
...
For example, if you want to find zeroes of a function, you really want to look at areas of the graph where the $y$ values are close to $0$. However, if you have a vertical asymptote on the same graph, the $y$ values might also be required to show very large or small values. With only a finite number of pixels available, it is impossible to easily do both.


What to do? If you were on a smartphone, you might be tempted to pan around to avoid the asymptotes, then pinch and zoom to narrow the graph to the feature of interest.  With `Julia` you basically do the same thing, though panning and zooming is done by changing the domain of the $x$ values used in plotting.
...
For our purposes, we want vectors (mathematically one dimensional, $n$ by 1 arrays.). In `Julia` vectors or functional replacements can be easily constructed in different ways, some discussed in the following.
...
Row and column vectors are different! We  use column vectors going forward.
...
The `range` function returns an object which is a sub-type of `AbstractVector`, so can be used where vectors are used, in most cases. The `show` method for ranges is such that the values are not displayed (nor are they computed until needed). To see the values, the abstract vector can be `collect`ed into a vector:

```{julia}
collect(range(0, pi/2, length=5))
```

Collecting values allows one to inspect them, but for most uses this is unnecessary as the functions these objects are passed to allow for the necessary abstraction which only requires an indexable containers.
...
Indexing can be used with assignment to modify a specific entry in a *mutable* array. The syntax is:

```{julia}
x[2] = 2
```

This assigns the second value of `x` a value of `2`---after conversion of `2` to the type of the array. There are mechanisms to assign more than one value not mentioned here.
...
`Julia` allows only three different types of expressions on the left side of an equals sign:
  ...
  * a assignment setting an index, as in `x[2] = 2`.
...
`Julia` allows a function to be broadcast over a collection of values with the simple notational trick of inserting a "." *before* the parenthesis used to call the function^[For *infix* functions, like `+`, the dot appears before the operator, as with `.+`.]. To see, we have:
...
Does this command produce the values $\{0.1, 0.01, 0.001, 0.0001\}$?
