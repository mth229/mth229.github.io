...
When adding multiple plots, there are a few concepts that can clarify the plot for the user:

* A legend is automatically included. The different things added can be included in the legend with a given name by passing the keyword argument `label="name to give"` to each layer.
...
For example, to directly create a 1-dimensional array with the `[]` syntax one simply type in the values separated by commas:
