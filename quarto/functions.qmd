...
* **Defining a function**, as above, can be done with the same syntax as mathematics. The function name with parentheses surrounds any arguments, and equal sign to indicated a *definition* and a rule on the right of the equals sign, indicating what becomes of the arugments.

* **Calling a function** Evaluating a function for a given value is also known in programming languages as "calling" `f`. Again, the `Julia` syntax follows the mathematical notation: the function name, with the *values* to define the function at enclosed in paretheses.
