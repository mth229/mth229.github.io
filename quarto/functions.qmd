...
We begin by loading some packages:^[Packages are a means to extend base `Julia`. There are thousands of add-on packages for `Julia`, but thees notes basically use just the two here, which rely of a handful of other add-on packages.]
...
A common use of multiple arguments in the `Julia` ecosystem is to use functions with the signature `f(x,p)` where the second argument is for a parameter (or parameters, when a container of values is passed).

The following use sets a *default* value for the parameter so it may or may note be specified:

```{julia}
f(x, p=1) = cos(x) - p * x
```

It isn't shown now, but this pattern can be used to solve for when $f(x,p) = 0$ for various choices of $p$ quite directly.

## Keyword arguments

Keyword arguments are widely used when plotting with `Julia` as they conveniently help customize specific attributes of a plot on a case-by-case basis. In the following, we see how keyword argument may also be used to specify parameters, though the pattern `f(x, p)`, as just described, is more commonly used.



We consider a simple case -- a function which computes the $y$ value on a line $y=mx+b$ from a given $x$ value. Here $m$ and $b$ are parameters. We will give them a default of $1$ and $0$:
...
The syntax is to use a semicolon  to separate regular arguments from those with keywords. This semicolon is not needed when calling the function; a comma may be used:
...
:::{.callout-note}
## `Base.Fix1` and `Base.Fix2`

The pattern of fixing a value of a function is common enough that there are also function `Base.Fix1` and `Base.Fix2`. This is a more performant means to *partially apply* the two-argument function `f` and appears often behind the scenes.
...
Julia is called a *dynamic programming language*  as variables can have their type changed. The *one* area where this is not possible is when a name is given to a generic function:
* the name of a generic function can be re-used as a variable name (`invalid redefinition of constant ...`)
* a variable name can be not reused as a name for a generic function (`cannot define function ...; it already has a value`)

Unfortunately this can happen easily with some names being used mathematically for both. For example, $h$ may be a common function name after $f$ and $g$ but is also a name for a value, as in the expression $f(c + h)$.
