...
We see in this section how to easily create mathematical functions in `Julia`. In the following sections we begin to do things with functions, such as graphing functions with `Julia`.
...
using Plots    # a package to provide ability to make plots
plotly()       # a selection of an interactive means to render plots
```
...
Once the environment is set up, for basic things, creating a new function and plotting it is as familiar as the following:
...
In the above, the first line defines a function---using the same notation used in mathematics; the second plots in a stratightforward manner only expecting the user to specify a function (by name) and a domain to plot over.
...
Mathematically, a function can be viewed in many different ways. An abstract means is to think of a function as a mapping, assigning to each $x$ value in the function's domain, a corresponding $y$ value in the function's range. With computer languages, such as `Julia`, the same holds, though there may be more than one argument to the function and with `Julia` the number of arguments and type of each argument are consulted to see exactly which function (technically method) is to be called.
...
We have three *different* things we do with functions:

* **Defining a function**, as above, can be done with the same syntax as mathematics. The function name with parentheses surrounds any arguments, an equal sign to indicate a *definition*, and a rule on the right of the equals sign, indicating what becomes of the arguments.

* **Calling a function** Evaluating a function for a given value is also known in programming languages as "calling" `f`. Again, the `Julia` syntax follows the mathematical notation: the function name, with the *values* to define the function at enclosed in parentheses.

* **Referring to a function** A function in `Julia` has a name, in the above this is just `f` (no parentheses, and definitely not `f(x)`.)


Though a function in `Julia` is typically *generic* and has different methods depending on the various inputs, this is a detail we don't  need to consider in these notes.

:::

A familiar example from physics which models the height of a projectile is

$$
h(t) = -\frac{1}{2}g \cdot t^2 + v_0\cdot t + x_0
$$

If we take $g=32$ and $v_0 = 1$ and $x_0 = 100$, we can quickly write a `Julia` function:

```{julia}
h(t) = -(1/2)*32 * t^2 + t + 100
```

If this models the height of an object thrown upwards with a certain velocity from a certain height, what is this height when $t=2$?
...
The convention is that when variables are immediately preceded by a numeric literal (and by immediately it is meant no spaces are in between) `Julia` will assume multiplication is intended.
...
But implicit multiplication won't work with expressions such as `(x+1)(x-1)`, as `Julia` will then think the first parenthetical pair is a function and try to call it with a value of `x-1` typically resulting in a `MethodError` and mention of something not being "callable".
...
The precedence of *implicit multiplication* is **different** from explicit multiplication. While convenient, this can cause confusion while learning. (The expressions `1/2x` and `1/2*x` are **not** the same, as the first evaluates `2x` before the division.) As such we recommend against the use of implicit multiplication at the outset.
...
will create a function which is called with a value of `x` at a later time. So the value of `x` defined when the function is created is not important here---`x` is passed in as an argument.
...
Mathematically, the horizontal bar in division implies grouping of the top and bottom. Where as the `/` operation does not. (It doesn't in mathematics either.) so mentally be prepared to do something like this:

$$
\frac{\cdots}{\cdots} \Rightarrow \text{  ( ⋯ ) / ( ⋯ ) }
$$
...
:::{.callout-note}
## `cbrt(x)` is not *always* `x^(1/3)`

In math we are used to turning $\sqrt[n]{x}$ into $x^{1/n}$. In `Julia` that can bump into problems when `x` is negative. The `cbrt` function works with negative numbers to return a negative number, whereas `x^(1/3)` will error when `x` is negative. The issue is $x^y$ might need a complex number to represent the answer for *certain* values of $y$, and `Julia` doesn't specialize its return value based on the value of `y`, but on the type of the value of `y`, in this case just throwing an error as some values of `y` would need complex numbers to represent the answer. (Starting with a complex number for `x` won't error.)
:::
...
To keep the number of functions to know small, these functions are avoided in the following, rather, when needed, converting from degrees to radians through multiplication by `pi/180` is used.
:::
...
The value of $e$ is built-in to `Julia`, but not immediately available. It is exposed by the `MTH229` package, so the above will work as long as that package has been loaded. But `e` is a convenient variable name, so `e` can be inadvertently redefined. As such, it is a safer practice to use the `exp` function, as in:
...
----
Let's do one more example which builds on the exponential function, the basic logistic function has this form

$$
f(x) = \frac{L}{1 + e^{-k\cdot(x - x_0)}
$$

Define a `Julia` function when $L=k=x_0=1$:
...
The logistic function is always increasing. If we compare the value at $1$ and $2$ we should see that:
...
A very common use of this is to represent an equation of the form $f(x) = g(x)$ as $h(x) = f(x) - g(x) = 0$. We might use this kind of code:
...
This names a function object; it does not create a method for a generic function (to be described later).

::: {.callout-note}
## Two types of names

One source of possible confusion is the name given to an anonymous function *can not* be used as a name for a function defined through the style `f(x) = ...`, as that creates a method for a *generic function*. Vice versa. That is, you can reuse names for variables for *other* variables, but not for new *methods* of generic functions. Further, you can reuse function (generic) names for other such, but not for variables. You will get an error if you try.
:::
...
Positional arguments work well for many cases, but are not always the most convenient approach:
...
* `fzero(f, c)` and `fzero(f, a, b)` will both find a zero of `f`, but the first form will look for one *near* `c`; the second for a zero between `a` and `b`---while using a *totally* different algorithm.
...
For example, the `plot` function, when called as below, expects functions of a single numeric variable. Behind the scenes, then the function `Area(w)` will be used in this graph:
...
The `Secant_Line` function above constructs and returns a *function*, so `sl` is a function object. We don't need to write `sl(x)` on the *left-hand side*, as we do when we define a generic function or method through an expression of its arguments. This *can* be done, were the following usage employed:

```{julia}
sec_line(x) = Secant_Line(f, a, b)(x)
```
...
The `secant(f, a, b)` function in the `MTH229` package returns a function like `Secant_Line` that has a reminder of what it does when used:
...
The `MTH229` package defines a related function `tangent(f, c)` to return a *function* which computes the tangent line to `f` at `c`:
...
The pattern of fixing a value of a function is common enough that there are also functions `Base.Fix1` and `Base.Fix2` (and more generally `Base.Fix{N}`). Applied to a function `f(x,y)`, `Fix1`  is used to create a function of `y` for a *fixed* `x`; `Fix2` to create a function of `x` for a fixed `y`. While the style of `SlopeIntercept` above, where an anonymous function is returned, is natural, the implementation of these two "fix" functions is more performant and their use appears often behind the scenes.
...
The extra bit `::Integer` is called a *type annotation* and specializes `n` and `k` so that this function only is called when both `n` and `k` are of this type.
...
```{julia}
plot(x^5 - x - 1, -1, 1.5)  # x is symbolic; Plots is loaded
```
