...
The `@symbolic` macro is used to create the variable, which is then used within other expressions:^[The `SymPy` package will be used later on in these notes. It too has symbolic variables, but thos variables have much more flexibility and the `SymPy` package has many, many more features. `SimpleExpressions` sole purpose is very modest, just to make basic expressions of a single variable and optional parameter act like functions.]
