...
However the *intuition* of limits is more accessible. This intuition was known as early as the Greeks: Archimedes figured out the area under a parabola over 2000 years ago by the method of exhaustion, a limiting process. Fermat, in 1629, essentially took a limit to find the slope of a tangent line to a polynomial curve. Newton, in the late 1600s, exploited the idea in his development of calculus (as did Leibniz). Yet it wasn't until the 1800s that [Bolzano, Cauchy, and Weierstrass](http://en.wikipedia.org/wiki/Limit_of_a_function#History) put the idea on a firm footing, as above.


To get the intuition behind a limit we replace the absolute value inequalities in the definition with "close" and read as follows: as $x$ gets "close" to $c$ (but not equal), then $f(x)$ should get "close" to $L$. We will see both visually and numerically what close means.


## Many limits are found just by evaluating the function


Before beginning, it should be noted that for most values of $c$, the answer is simply $L=f(c)$. This is because *most* all the functions encountered will be continuous which is basically a statement that for any $c$ the limit exists and is given through $L=f(c)$.
...
However, at $c=0$ we can't say $f(x)$ is continuous---it isn't even defined. When $c$ is non-zero, the function $f(x)$ is simply the ratio of two continuous functions. Such a ratio will be continuous except when dividing by $0$, as is the case only when $c=0$
...
The value `NaN` arises when some floating-point computations are indeterminate.
...
:::{.callout-note}
## `NaN` values
Operations involving `NaN` will also return `NaN`.  In this example, the ratio defining $f(x)$ is like $0/0$ when $c=0$---an indeterminate form. The value `NaN` is returned by *most* of the indeterminate forms listed above, but not all. For example, `0^0` is `1` to `Julia`: "It proved more useful to adhere to the simply stated rule *anything raised to the $0$ gives $1$ then special case $0^0$. Some comments by the "father of floating point" can be read [near the end of this](http://people.freebsd.org/~das/kahan86branch.pdf)."
...
```{julia}
#| fig-alt: plot of sinc function, sin(x)/x near c=0
f(x) = sin(x)/x
c, delta = 0, pi/2
plot(f, c - delta, c + delta)
```

From the graph it looks like the function value should be $1$ when $x=0$. This is indeed the case, as was understood as early as 1735 by Euler.
...
```{julia}
#| fig-alt: plot of rational function near a removable discontinuity
c, delta = 2, 1
plot(f, c - delta, c + delta)
```
...
answer = 1
explanation = raw"""
The squeeze theorem allows one to find the limiting value of $f(x)$ if the limiting values of $g(x)$ and $h(x)$ are *known$, *equal*, and $g(x) \le f(x) \le h(x)$. This is because $f$ is squeezed between $g$ and $h$ as they each get close to $L$.
"""
buttonq(choices, answer; explanation)
```
...
The three steps above are  bit tedious to type for each problem, so for convenience we encapsulate them into a function (available in the `MTH229` package) called `lim`:
...
is notation to indicate that one can take a limit of the sum of two functions or take the limit of each first, then add and the answer will be unchanged, provided all the limits in question exist.
...
annotate!([(x, y, text(L"\theta",:center)),
           (1/3, 1/3 * sin(θ) + 0.1, text(L"1")),
           ])
```
...
annotate!([(1, sin(t/2), text(L"k",:left))
           ])
```
...
The sum $1 + 1/2 + 1/3 + 1/4 + \cdots$ does not converge. In fact, the sum of the first $n$ terms gets closer and closer to $\log(n)$ plus a constant. That is, the following function does have a [limit](http://en.wikipedia.org/wiki/Euler%E2%80%93Mascheroni_constant) as $n$ goes to $\infty$:
...
Related in that they are used to approximate related functions near $0$: $\sin(x) \approx x$ and $1 - \cos(x) \approx (1/2) x^2$. The latter apporoximation can be seen in  #fig-cos-parabola-0.


```{julia}
#| echo: false
#| label: fig-cos-parabola-0
...
measures how much smaller the representable number *just before* $1.0$ is from $1.0$. (Of course, this has no answer in the real numbers, but floating point is a discrete approximation.)
...
`Julia` has some functions for working with floating point numbers. We have `nextfloat(x)` and `prevfloat(x)` returning the floating point numbers just *bigger* than the floating point representation of `x` and just *smaller* than `x`. The value of `eps(x)` is `nextfloat(x) - x` or the *gap* between `x` and its adjacent floating point number that is just bigger. The value `eps()` is just `eps(1.0)`. If you were to compare `eps(prevfloat(1.0))` to `eps(1.0)` you would see the latter is twice the former. These *gaps* double between the intervals $\dots, [1/2,1), [1,2), [2, 4), \dots$.


:::


The above issue with $1-\cos(x)$ would happen when $\cos(x)$ is within `1.1102230246251565e-16` of `1`. As $\cos(x) is like $x^2/2$ this should be around `1e-8` and we see that is the case here:
...
and this explains the `0` among these two numbers:

```{julia}
1 - cos(1e-7), 1 - cos(1e-8)
```
...
The `limit` function has one named argument, `dir`, used to adjust if a left or right (the default) limit is sought. For example, this function has different left and right limits at 0:
...
$$
\lim_{x \rightarrow \pi/2} \frac{cos(x)}{\pi/2 - x}
$$
...
**However**, the astute student will see this as related to the limit of $\sin(x)/x$ at $0$ which is of course $1$, and so the above is *incorrect*.


This is due to `pi/2` being converted to floating point. This eager conversion can be avoided by using `PI`, from `SymPy`:
...
`PI` is a symbolic variable to represent $\pi$; `E` a symbolic variable to represent $e$; and `oo` a symbolic variable to represent $\infty$. Alternatively, one can make `2` symbolic and divide that into `pi` with `pi/Sym(2)`. Either way, if `pi/2` is evaluated with both values being non-symbolic, then a floating point **approximation** to the mathematical $\pi/2$ is used.
...
Here is how we define two symbolic values:
