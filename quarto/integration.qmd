...
For pedagogical use, a function like this is defined in the `MTH229` package:
...
Two values are returned, the answer and an estimate of the error. In the above, $2$ is the exact answer to this integral, the estimated value a just a bit more than $2$, but is estimated to be off my no more than the second value, $1.78 \cdot 10^{-12}$.
...
With the formula for the volume of a solid of revolution we can compute these marks numerically if we know the radius as a function of height.
...
![Beer glasses---and not one traditional pint glass](figures/integration/beer_glasses.jpg)
...
Not to worry, we can use `fzero` from the `Roots` package for that (again, this is loaded with the `MTH229` package). To solve for when `V(b) = r_vol(b) - 450 = 0` we have
...
![Jasper Johns, Near the Lagoon](figures/integration/johns-catenary.jpg){fig-alt: image of Jasper Johns Near the Lagoon showing a catenary shape}
...
In a picture of the Verrazano-Narrows [bridge](http://observer.com/2012/09/staten-island/s-i-8/), would the shape during construction be a parabola or a catenary?
...
The Gauss nodes and weights are computable [(Gaussian quadrature)](http://en.wikipedia.org/wiki/Gaussian_quadrature). The main tools are the so-called Legendre polynomials, which can be defined recursively with Bonnet's formula:
