...
Here is a similar, though more complicated, example where the analytic approach can be a bit more tedious, but the graphical one mostly satisfying, though we do use  the `fzero` function to find the exact final answer.
...
annotate!([
    (0, -0.1, text(L"x", :bottom)),
    (1.1, 1, L"y"),
    (0.6, 2 + 0.4*sin(pi/4), text(L"x/2", :bottom))
])
```
...
annotate!([(a+Δ, b/2, text(L"8")),
           (a + b*tan(θ) - 3Δ, b/2, text(L"l_1")),
           (a/2, b-Δ, text(L"5")),
           (a/2, b + a/2*tan(θ) - 2Δ, L"l_2"),
           (last(xlims)-4Δ, 0 + 3Δ/2, text(L"\theta"))
           ])
```
...
That is, any ladder less than this length can get around the two-dimensional hallway.
...
Rather than use a bracketing method to solve for the root of $l'(t)=0$, we might have tried using `fzero(l', 0.8)`. Do so. What do you find?
...
t = fzero(A', pi/4, Roots.Order16());	## Has issues with order=8 algorithm, tol > 1e-14 is needed
val = t * 180/pi;
numericq(val, 1e-3)
```
...
annotate!([(x/2, 0 + 3Δ, L"x"),
           (x + 3Δ, 10/2, L"10"),
           (x + 3Δ, 10 + 20/2, L"20"),
           (x/4, x/4 * 10/x + 5Δ, L"\theta")
           ])
...
It appears to be around 8.3. We now use `fzero` to refine our guess at the critical point using a bracketing algorithm:
...
The minimum looks to be around 4cm. We can use `fzero` to zero in on the answer:
...
annotate!([
    ...
    (1,1,text(L"(a,b)",:bottom)),
    (x[3]/2, 0, text(L"x",:top)),
    (x[3]-Δ, y[3]/2, text(L"|y|",:right)),
    (x[3] + Δ, (1 + y[3])/2, text(L"b-y",:left)),
    ((x[3]+1)/2, 1, text(L"a-x",:top))
])
...
function g(x)
   h(y) = f(x, y) - Len
   fzero(h, zero(x))  # not 0, but zero(x) to get type right
end
```
...
Okay, now to find the lowest point. This is what we minimize to find the resting position of the ring. Again, we turn to `fzero`, but for minimization we use the first derivative, as we are finding critical points.
