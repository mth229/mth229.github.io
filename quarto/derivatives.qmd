...
plot(f, -1, 1, legend=false, ylim=(1,3))
plot!(secant(f, c, c + 1))
plot!(secant(f, c, c + 3/4))
plot!(secant(f, c, c + 1/2))
plot!(secant(f, c, c + 1/4))
```
...
The marginal profit is the change in profit per unit sold---or the derivative---at $x=200$.

We can find this quite simply by either differentiating directly, or as we do here, approximating the answer with `Julia`. We first note that the function above, is a composition, so it may be written using two functions:
...
The built-in `airyai` function, when called as `airyai(x)`, returns the so-called Airy function, a special function named after [George Biddell Airy](http://en.wikipedia.org/wiki/Airy_function).
...
From the relationship of the derivative and the function, we know the answer is when $f'(x) > 0$. (This is formalized using the mean value theorem.) For this example, we can solve this directly as $f'(x) = -\sin(x)$ and we know when that is positive (well you are supposed to anyways: when the angle is in the third and fourth quadrants). Let's see how we would do this task with `Julia` and compare.
...
When we look, we see that for awhile the approximation gets better (to about `1e-9`), but then does a U-turn and starts getting worse. The mathematical approximation gets better and better, but what happens is the computational error gets worse.  Recall this rule of thumb for floating point mathematics:

> If we subtract two like-sized quantities our answer may have dramatically reduced precision.


Of course $f(c+h)$ and $f(c)$ are two like-sized quantities for small $h$---as $f(x)$ is *continuous* if it is differentiable. We'll see below how to pick the $h$ that best balances these errors off, but first lets look at how using different approximations for the derivative can improve the "mathematical" error.


### Central difference quotient


It turns out that just by looking to the left and the right we can improve the mathematical error from getting better at a rate of $h$ to a rate of $h^2$, provided our function is smooth enough and we don't have issues on the boundaries. The formula we use, called the *central difference quotient* approximation to the derivative, is:
...
Let's compare to see the error is like $h^2$, not $h$. To make our life easier we again create some functions, as we did with `Df` above.
...
Then this function could easily be reused for other problems.
...
So at $12.41\cdots$ days, the instantaneous rate is equal to the average rate of decomposition over the first 30 days.
...
Which definition would be right for  $(x,dx) - (y, dy)$ following the examples on forward mode automatic differentiation?
...
Where $M$ is a constant that depends on the maximum absolute value of the second derivative and the $1/2$ comes from second semester calculus. Choosing $h$ to make this as small as possible is possible with some calculus involving the derivative, but for now we simply note the answer is $h=((2\epsilon)/(M/2))^{1/2} = 2\sqrt{\epsilon}/\sqrt{M}$.
...
$$
h_{min} = 2\sqrt{epsilon}/1 \approx 10^{-8}
$$
...
The `SymPy` package can differentiate a symbolic expression and return a symbolic expression, similar to what one would get by hand.

The key function is `diff(ex)` or `diff(ex, x)` when specifying which variable to differentiate in is needed. Higher order derivatives can be computed by repeating the variable (eg. `diff(ex, x, x)`) or specifying a number after the variable (eg. `diff(ex, x, 2)`),


Consider the function $f(x) = x \cdot \tanh(\exp(x))$. We define it and then create a symbolic expression:
...
This expression has first and second derivatives computed by:
...
If there are one or more parameters, the variable must be specified.
